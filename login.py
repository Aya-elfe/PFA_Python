...
def get_user_by_username(username, is_admin=False):
...
def login_user():
    ...
    user = get_user_by_username(username, is_admin)

    if not user:
        return

    hashpassword = password.encode('utf-8')
    stored_password = user.password.encode('ascii')

    if bcrypt.checkpw(hashpassword, stored_password):
        root.destroy()
        import add1
    else:
