...
def on_update_click(livre):
    ...
    update_button = tk.Button(update_window, text="Modifier", font=("Helvetica", 14), padx=10, pady=5, command=submit_update)
    update_button.pack()
...
for i, livre in enumerate(livres):
    ...
    card_button.pack(padx=5, pady=5, side=tk.RIGHT)
    # Create edit card button and on_update_click() function
    card_button = tk.Button(card_frame, image=mod, bg='#FAF8F8', border=0, activebackground='#FAF8F8',command=lambda livre=livre: on_update_click(livre))
